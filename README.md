...
### 🗃️ SQL 로그 변환 (`sql-converter.html`)
- 로그에 찍힌 SQL의 플레이스홀더(`?`)를 실제 값으로 치환해 **실행 가능한 쿼리**로 만듭니다.
- **MyBatis 로그 자동 분리**: `Preparing:`/`Parameters:`가 포함된 로그 블록을 통째로 붙여넣으면 SQL과 파라미터를 자동 추출.
- **타입 인식 바인딩**: `홍길동(String)`·`123(Integer)`·`null` 등 MyBatis 타입을 읽어 따옴표를 자동 처리(문자열 내 `'`는 `''`로 이스케이프). `[a, b, c]` 배열 형식도 지원.
- **개수 검증**(`?` 개수 vs 파라미터 개수 불일치 경고), 자바 문자열 결합/주석 정제, **SQL 포맷터**, 파일 드래그앤드롭, Ctrl+Enter 변환.
